...
import { deleteFunction, deployFunction, invokeFunction, checkFunctionHealth } from './faasApi.ts';
import logger from '../../../logger.ts';
import { RetryRequestError, RespStatusError } from '../utils.ts';
import stats from '../../../utils/stats.ts';
import { getMetadata, getTransformationMetadata } from '../../../v0/util.ts';
import { HTTP_STATUS_CODES } from '../../../v0/constants.ts';

const FAAS_SCALE_TYPE = Deno.env.FAAS_SCALE_TYPE || 'capacity';
const FAAS_SCALE_TARGET = Deno.env.FAAS_SCALE_TARGET || '4';
const FAAS_SCALE_TARGET_PROPORTION = Deno.env.FAAS_SCALE_TARGET_PROPORTION || '0.70';
const FAAS_SCALE_ZERO = Deno.env.FAAS_SCALE_ZERO || 'false';
const FAAS_SCALE_ZERO_DURATION = Deno.env.FAAS_SCALE_ZERO_DURATION || '15m';
const FAAS_BASE_IMG = Deno.env.FAAS_BASE_IMG || 'rudderlabs/openfaas-flask:main';
const FAAS_MAX_PODS_IN_TEXT = Deno.env.FAAS_MAX_PODS_IN_TEXT || '40';
const FAAS_MIN_PODS_IN_TEXT = Deno.env.FAAS_MIN_PODS_IN_TEXT || '1';
const FAAS_REQUESTS_CPU = Deno.env.FAAS_REQUESTS_CPU || '0.5';
const FAAS_REQUESTS_MEMORY = Deno.env.FAAS_REQUESTS_MEMORY || '140Mi';
const FAAS_LIMITS_CPU = Deno.env.FAAS_LIMITS_CPU || FAAS_REQUESTS_CPU;
const FAAS_LIMITS_MEMORY = Deno.env.FAAS_LIMITS_MEMORY || FAAS_REQUESTS_MEMORY;
const FAAS_MAX_INFLIGHT = Deno.env.FAAS_MAX_INFLIGHT || '4';
const FAAS_EXEC_TIMEOUT = Deno.env.FAAS_EXEC_TIMEOUT || '4s';
const FAAS_ENABLE_WATCHDOG_ENV_VARS = Deno.env.FAAS_ENABLE_WATCHDOG_ENV_VARS || 'true';
const CONFIG_BACKEND_URL = Deno.env.CONFIG_BACKEND_URL || 'https://api.rudderlabs.com';
const GEOLOCATION_URL = Deno.env.GEOLOCATION_URL || '';
export const FAAS_AST_VID = 'ast';
export const FAAS_AST_FN_NAME = 'fn-ast';
const CUSTOM_NETWORK_POLICY_WORKSPACE_IDS = Deno.env.CUSTOM_NETWORK_POLICY_WORKSPACE_IDS || '';
const customNetworkPolicyWorkspaceIds = CUSTOM_NETWORK_POLICY_WORKSPACE_IDS.split(',');
const CUSTOMER_TIER = Deno.env.CUSTOMER_TIER || 'shared';
...
export async function setupFaasFunction(
  functionName,
  code,
  versionId,
  libraryVersionIDs,
...
export const executeFaasFunction = async (
  name,
  events,
  versionId,
  libraryVersionIDs,

...
import { compileUserLibrary } from './ivmFactory.ts';
import { getTransformationCode } from './customTransformationStore.ts';
import { getTransformationCodeV1 } from './customTransforrmationsStore-v1.ts';
import { UserTransformHandlerFactory } from './customTransformerFactory.ts';
import { parserForImport } from './parser.ts';
...
async function runUserTransform(
  ...
  await jail.set('_rsSecrets', (...args) => {
  ...
  try {
    ...
    const tags = {
      ...
      ...(events.length > 0 && events[0].metadata ? getMetadata(events[0].metadata) : {}),
      ...(events.length > 0 && events[0].metadata ? getTransformationMetadata(events[0].metadata) : {}),
    };
...
export async function userTransformHandler(
  events,
  versionId,
  libraryVersionIDs,
  trRevCode = {},

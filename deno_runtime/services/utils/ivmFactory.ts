...
import { getLibraryCodeV1, getRudderLibByImportName } from './customTransforrmationsStore-v1.ts';

import { extractStackTraceUptoLastSubstringMatch, fetchWithDnsWrapper } from './utils.ts';
...
async function createIvm(code, libraryVersionIds, versionId, secrets, testMode) {
  ...
  if (code && libraries) {
    const extractedLibraries = Object.keys(
      ...
      await extractLibraries(code, null, false, [], 'javascript', testMode),
    );
  ...
  await jail.set('_rsSecrets', (...args: any) => {
...
export async function compileUserLibrary(code) {
  const isolate = new ivm.Isolate({ memoryLimit: ISOLATE_VM_MEMORY });
  const context = await isolate.createContext();
  return evaluateModule(isolate, context, code);
}

export async function getFactory(code, libraryVersionIds, versionId, secrets, testMode) {
  const factory = {
    create: async () => createIvm(code, libraryVersionIds, versionId, secrets, testMode),
